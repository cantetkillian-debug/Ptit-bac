...
function botDifficultyConfig(level) {
  const configs = {
    easy:   { minThink: 1200, maxThink: 2800, submitBuffer: 6500, answerChance: 0.72, mistakeChance: 0.15 },
    normal: { minThink: 800,  maxThink: 2200, submitBuffer: 4200, answerChance: 0.88, mistakeChance: 0.07 },
    hard:   { minThink: 500,  maxThink: 1500, submitBuffer: 2600, answerChance: 0.97, mistakeChance: 0.025 }
  };
  return configs[level] || configs.normal;
}

function makeBotMistake(answer, letter) {
  if (!answer) return "";
  // Quelques erreurs naturelles pour que les bots ne soient pas parfaits.
  if (answer.length > 4 && Math.random() < 0.65) {
    const i = Math.max(1, Math.min(answer.length - 2, 1 + Math.floor(Math.random() * (answer.length - 2))));
    return answer.slice(0, i) + answer.slice(i + 1);
  }
  return Math.random() < 0.5 ? "" : `${letter}test`;
}
...
function playBots(room) {
  ...
  const cfg = botDifficultyConfig(room.botDifficulty || "normal");
  const roundDurationMs = Math.max(10000, Number(room.duration || 60) * 1000);

  bots.forEach((bot, botIndex) => {
    if (!bot.answers[roundIndex]) bot.answers[roundIndex] = {};

    let elapsed = 650 + botIndex * 300 + Math.floor(Math.random() * 550);
    room.categories.forEach((category, categoryIndex) => {
      const step = cfg.minThink + Math.random() * (cfg.maxThink - cfg.minThink);
      elapsed += step;
      const fireAt = Math.min(elapsed, Math.max(1200, roundDurationMs - cfg.submitBuffer - (room.categories.length - categoryIndex) * 250));

      setTimeout(() => {
        const current = rooms.get(room.code);
        if (!current || current.phase !== "round" || current.roundIndex !== roundIndex || bot.submitted) return;

        let answer = "";
        if (Math.random() <= cfg.answerChance) {
          answer = makeBotAnswer(category, letter);
          if (Math.random() < cfg.mistakeChance) answer = makeBotMistake(answer, letter);
        }
        bot.answers[roundIndex][category] = answer;
        emitRoom(current);
      }, fireAt);
    });
    ...
    const submitAt = Math.min(
      roundDurationMs - 900,
      Math.max(elapsed + 500 + Math.random() * 1400, roundDurationMs * (0.58 + Math.random() * 0.28))
    );
...
io.on("connection", socket => {
  ...
  socket.on("room:addBot", payload => {
    const { room, player } = requireMember(socket, payload);
    if (!room || !player?.isHost || room.phase !== "lobby") return;
    if (room.players.length >= 12) return socket.emit("toast", "Le salon est complet.");
  ...
  socket.on("room:updateSettings", ({ code, playerId, rounds, duration, botDifficulty }) => {
    const { room, player } = requireMember(socket, { code, playerId });
    if (!room || !player?.isHost || room.phase !== "lobby") return;
    if ([1, 3, 5].includes(Number(rounds))) room.rounds = Number(rounds);
    if ([30, 60, 90].includes(Number(duration))) room.duration = Number(duration);
    if (["easy", "normal", "hard"].includes(botDifficulty)) room.botDifficulty = botDifficulty;
